...
export interface EasyExplanation {
  ...
  analogy: string;         // 비유 설명 (초등학생도 이해할 수 있는)
  visualAid: string;       // 이모지/ASCII 그림
  commonMistakes?: string; // 헷갈리는 포인트
}
...
export const studyItems: StudyItem[] = [
  // ==================== 암호학 ====================
  {
    ...
    easyExplanation: {
      oneLiner: "어떤 파일이든 정해진 길이의 '지문'으로 바꿔주는 마법!",
      analogy: `사람마다 손가락 지문이 있죠?
지문은 아무리 키가 크든 작든 항상 비슷한 크기예요.

해시 함수도 똑같아요!
1GB 영화 파일이든, "안녕" 두 글자든
모두 정해진 길이의 "디지털 지문"을 만들어요!

이 지문으로 뭘 할 수 있냐고요?
• 파일이 바뀌었는지 확인할 수 있어요
• 비밀번호를 안전하게 저장할 수 있어요
• 원본으로 되돌릴 수는 없어요 (일방통행!)`,
      visualAid: `
┌────────────────────────────────────────────────┐
│         🔮 해시 함수 = 디지털 지문기계            │
├────────────────────────────────────────────────┤
...
│   📝 1글자 "A"       ───→  🔮  ───→  i9j0k1l2  │
│                                                │
│   ⬆️ 입력 크기 다름     ⬆️ 출력 크기 같음!       │
│                                                │
├────────────────────────────────────────────────┤
│   📏 출력 크기 정리:                            │
│                                                │
│   MD5    → 128bit (🚨 취약! 쓰지마세요)        │
│   SHA-1  → 160bit (🚨 취약! 쓰지마세요)        │
│   SHA-256 → 256bit (✅ 안전! 추천)              │
│   SHA-512 → 512bit (✅ 더 안전!)                │
│                                                │
└────────────────────────────────────────────────┘`,
      commonMistakes: `• MD5가 안전하다?
  → ❌ MD5는 이미 깨졌어요! SHA-256 사용하세요

• 해시에서 원본을 복구할 수 있다?
  → ❌ 불가능! 일방통행이에요 (지문에서 얼굴을 알 수 없듯이)

• SHA-256의 출력이 256byte?
  → ❌ 256bit예요! (32byte = 64개 16진수 문자)`
    },
    content: `
**해시 함수란?**
...
- **ECDH**: 타원곡선 기반 (더 짧은 키로 동일 보안)
  ...
  // ==================== Linux ====================
  {
    ...
    easyExplanation: {
      oneLiner: "컴퓨터 사용자들의 명단이 적힌 출석부!",
      analogy: `학교에 가면 선생님이 출석부를 가지고 있죠?
이름, 번호, 집 주소, 전화번호가 적혀 있어요.
...
근데 "비밀번호"는 여기 없어요!
왜? 출석부는 다 볼 수 있으니까 비밀번호를 여기 쓰면 위험하죠!`,
      visualAid: `
┌─────────────────────────────────────────────────┐
│     📋 /etc/passwd = 컴퓨터 출석부               │
├─────────────────────────────────────────────────┤
│                                                 │
│  👤 kim : x : 1000 : 1000 : 김철수 : /home/kim : /bin/bash
│   ↑     ↑    ↑      ↑       ↑        ↑           ↑
│  이름  비번  번호   반번호   설명     집          쉘
│       (숨김)                                    (대화창)
│                                                 │
│  x = "비밀번호는 다른 곳(shadow)에 있어요!"      │
│                                                 │
│  ⚠️ UID가 0이면? → 반장(root)이에요!             │
│                                                 │
└─────────────────────────────────────────────────┘`,
      commonMistakes: `• "x"가 비밀번호인 줄 알았어요
  → ❌ x는 "shadow 파일 보세요"라는 표시예요!

• UID 0은 일반 사용자?
  → ❌ UID 0 = root(관리자)! 최고 권한!

• 비밀번호가 이 파일에 있다?
  → ❌ 옛날엔 있었지만, 지금은 /etc/shadow에!`
    },
    content: `
**파일 구조:**
\`\`\`
...
\`\`\`
  ...
  },
  {
    ...
    easyExplanation: {
      oneLiner: "누가 언제 왔다갔는지 기록하는 출입 기록부!",
      analogy: `학교 정문에 경비 아저씨가 계시죠?
아저씨가 기록하는 것처럼 리눅스도 기록해요!
...
│  📋 utmp (/var/run/utmp)                        │
│  ┌───────────────────────────────┐              │
│  │ 현재 로그인: kim, lee, park   │  ← who 명령어│
│  └───────────────────────────────┘              │
│                                                 │
│  📚 wtmp (/var/log/wtmp)                        │
│  ┌───────────────────────────────┐              │
│  │ 3/15 09:00 kim 로그인         │              │
│  │ 3/15 10:30 kim 로그아웃       │  ← last 명령어
│  │ 3/15 11:00 lee 로그인         │              │
│  └───────────────────────────────┘              │
│                                                 │
│  🚫 btmp (/var/log/btmp)                        │
│  ┌───────────────────────────────┐              │
│  │ 3/15 02:00 admin 실패 ❌      │              │
│  │ 3/15 02:01 root 실패 ❌       │  ← lastb 명령어
│  │ 3/15 02:02 admin 실패 ❌      │   (해킹 시도?)
│  └───────────────────────────────┘              │
│                                                 │
│  ⚠️ 모두 바이너리! cat으로 못 봐요!               │
│                                                 │
└─────────────────────────────────────────────────┘`,
      commonMistakes: `• cat /var/log/wtmp 하면 볼 수 있다?
  → ❌ 바이너리라서 깨져 보여요! last 명령어 쓰세요

• utmp가 로그인 기록이다?
  → ❌ utmp는 "현재" 로그인만! 기록은 wtmp!
...
- 다중 인증 (MFA) 적용
  ...
  // ==================== 네트워크 ====================
  {
    ...
    easyExplanation: {
      oneLiner: "도청, 사칭, 납치, 몰래숨기기 - 나쁜 해커의 4가지 수법!",
      analogy: `학교에서 일어날 수 있는 나쁜 일로 비유해볼게요!

🎧 Sniffing (스니핑) = 도청
  "야, 쟤네 뭐라고 얘기해?" 몰래 귀 기울여 듣기

🎭 Spoofing (스푸핑) = 사칭
  "나 반장이야~" 다른 사람인 척하기

🏃 Hijacking (하이재킹) = 납치
  "이 대화 내가 이어받을게" 진행 중인 대화 가로채기

🚇 Tunneling (터널링) = 몰래 숨기기
  "이 편지 안에 쪽지 숨겨서 보내자" 몰래 다른 걸 보내기`,
      visualAid: `
┌──────────────────────────────────────────────────┐
│      🎯 네트워크 공격 4대 유형                     │
├──────────────────────────────────────────────────┤
...
│                                                  │
└──────────────────────────────────────────────────┘`,
      commonMistakes: `• Sniffing이 능동적 공격?
  → ❌ 수동적 공격! 그냥 듣기만 해요

• Spoofing과 Hijacking이 같다?
  → ❌ Spoofing=처음부터 사칭, Hijacking=진행 중 가로채기
